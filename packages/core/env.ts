...
export const tsapp = new ENV(
    zod.object({
        ...
        TSAPP_PORT: zod.coerce.number().int(),
        ...
        TSAPP_NEXT_PORT: zod.coerce.number().int().default(8010).optional(),
        TSAPP_API_PORT: zod.coerce.number().int().default(8011).optional(),
        TSAPP_GRAPHQL_PORT: zod.coerce.number().int().default(8013).optional(),
    }),
    {
        TSAPP_ENV_TYPE: "dev",
        TSAPP_PORT: 8000,
...
export const haproxy = new ENV(
    zod.object({
        ...
        HAPROXY_TSAPP_PORT: zod.coerce.number().int(),
    }),
    {
...
export const certbot = new ENV(
    zod.object({
        ...
        CERTBOT_PORT: zod.coerce.number().int(),
...
export const mongo = new ENV(
    zod.object({
        MONGO_PORT: zod.coerce.number().int(),
...
export const rabbitmq = new ENV(
    zod.object({
        RABBITMQ_PORT: zod.coerce.number().int(),
...
export const sftp = new ENV(
    zod.object({
        SFTP_PORT: zod.coerce.number().int().default(22),
...
export const wireguard = new ENV(
    zod.object({
        WIREGUARD_PORT: zod.coerce.number().int().default(51820),
...
export const codeserver = new ENV(
    zod.object({
        CODESERVER_PORT: zod.coerce.number().int().default(8015),
